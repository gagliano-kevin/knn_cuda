...
int main(int argc, char** argv) {
    ...
    double exeTimes[10];                                                                                        // Execution times for each k value
    ...
    for(k = 5; k <= 50; k += 5){
        ...
        appendRunStatsToFile("artificial_k_cu.txt", "artificial_k/", runTimes, 5);
        appendResultsToFile(errorCount, testSize, "artificial_k_cu.txt", "artificial_k/", trainSize, num_features, k, metric, exp, distDim, predDim, workers, alpha, beta, avgKnnDistElaps, avgKnnElaps, sharedMemorySize, maxSharedMemory, sharedWorkers);

        exeTimes[(k/5)-1] = avgKnnElaps + avgKnnDistElaps;                                                      // Store the execution time for the current k value
