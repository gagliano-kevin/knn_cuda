...
__global__ void knnSortPredict(double *distances, int trainSize, int *indexes, int k, int *predictions, int *trainLabels) {
    ...
    if(threadIdx.x == blockDim.x - 1){
        portion = trainSize - (blockDim.x - 1) * portion;
    }
    ...
    // write to shared memory k nearest points and their indexes
    //__shared__ double sharedDistances[k * blockDim.x]; 
    //__shared__ int sharedIndexes[k * blockDim.x];
    __shared__ double sharedDistances[5 * 5]; 
    __shared__ int sharedIndexes[5 * 5];
